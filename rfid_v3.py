...
dev = map(InputDevice, ('/dev/input/event3', '/dev/input/event0'))
...
cur.execute('REPLACE INTO employee values(0001, "2121743110", "Jack Aranda")')
cur.execute('REPLACE INTO employee values(0002, "2121723560", "Cherry Blossom")')
cur.execute('REPLACE INTO employee values(0003, "2121741279", "Olive Tree")')
                                    ...
                                    if len(code) >= 10:
                                        ...
                                        if not employee:                
                                            ...
                                            if not employeeStatus or employeeStatus[0] == 0:
                                                ...
                                                GPIO.output(RED_LED, False)
                                                GPIO.output(BLUE_LED, True)
                                                print 'you have already been scanned in, please scan out first'
                                                con.commit()
                                except:
                                    code = []
                                    rfid_data = ""
                                ...
                                try:
                                    ...
                                    if len(code) >= 10:
                                        ...
                                        if not employee:                
                                            ...
                                            if not employeeStatus or employeeStatus[0] == 1:
                                                cur.execute("INSERT INTO tracking (Card, Date, Status) VALUES(?,?,?)", (rfid_data, time.time(), 0))
                                                print employee[1],"left area @", thetime
                                            elif employeeStatus[0] == 0:
                                                GPIO.output(BLUE_LED, False)
                                                GPIO.output(RED_LED, True)
                                                time.sleep(2)
                                                GPIO.output(RED_LED, False)
                                                GPIO.output(BLUE_LED, True)
                                                print 'You have already been scanned out please scan in first' 
                                                con.commit()
                                except:
                                    code = []
                                    rfid_data = ""
